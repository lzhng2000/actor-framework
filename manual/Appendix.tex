...
\subsection{Class \texttt{maybe}}
\label{Appendix::maybe}

Defined in header \lstinline^"caf/maybe.hpp"^.
...
Represents an optional value or error.
...
\begin{tabular*}{\textwidth}{m{0.5\linewidth}m{0.45\linewidth}}
  ...
  \hline
  \lstinline^errortype^ & \lstinline^error^ \\
  \hline
  \\
  \multicolumn{2}{l}{\large{\textbf{Member Functions}}\vspace{3pt}} \\
  \hline
  \lstinline^maybe()^ & Constructs an empty maybe \\
  \hline
  \lstinline^maybe(T)^\newline\lstinline^maybe& operator=(T)^& Initialize or set a value \\
  \hline
  \lstinline^maybe(error)^\newline\lstinline^maybe& operator=(error)^& Initialize or set an error \\
  \hline
  \lstinline^maybe(const maybe&)^\newline\lstinline^maybe& operator=(const maybe&)^& Copy construct/assign \\
  \hline
  \lstinline^maybe(maybe&&)^\newline\lstinline^maybe& operator=(maybe&&)^& Move construct/assign \\
  \hline
  \\
  ...
  \hline
  \lstinline^bool empty()^ & Returns \lstinline^true^ if \lstinline^this^ is default-constructed \\
  \hline
  \lstinline^bool invalid()^ & Returns \lstinline^true^ if \lstinline^this^ has an error \\
  \hline
  \lstinline^bool operator!()^ & Returns \lstinline^!valid()^ \\
  \hline
  \lstinline^const T& get()^\newline\lstinline^const T& operator*()^ & Returns the stored value \\
  \hline
  \lstinline^error_type error()^ & Returns the stored error \\
  \hline
  \\
  \multicolumn{2}{l}{\textbf{Modifiers}\vspace{3pt}} \\
  \hline
  \lstinline^T& get()^\newline\lstinline^T& operator*()^ & Returns the stored value \\
  \hline
\end{tabular*}
}
...
In case you were using the old \lstinline^cppa::maybes_description^ API, you can migrate to the new API based on \lstinline^extract^ (cf. \ref{extract-opts}).
...
Their replacements are named \lstinline^current_message^ and \lstinline^current_sender^ (see \S\,\ref{interface}).
